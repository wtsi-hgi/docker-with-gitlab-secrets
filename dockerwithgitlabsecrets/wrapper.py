...
_SUPPORT_DOCKER_ACTIONS = ["exec", "run"]
...
def run_wrapped(docker_arguments: List[str], project_variables_manager: ProjectVariablesManager,
    ...
    while docker_arguments[docker_action_index] not in _SUPPORT_DOCKER_ACTIONS:
        docker_action_index += 1
        if docker_action_index == len(docker_arguments):
            # FIXME: Just change to ignore to allow aliasing
            raise ValueError("This wrapper only supports the Docker `run` command")
    ...
    with NamedTemporaryFile(suffix=_DOCKER_ENV_FILE_SUFFIX, mode="w") as env_file:
        ...
        docker_call = [_DOCKER_BINARY] + docker_arguments[0:docker_action_index+1] + \
                      [f"--{ENV_FILE_PARAMETER}", env_file.name] + docker_arguments[docker_action_index+1:]
